function I(e) {
  return Array.isArray ? Array.isArray(e) : et(e) === "[object Array]";
}
const at = 1 / 0;
function lt(e) {
...
function ft(e) {
  return e === !0 || e === !1 || (dt(e) && et(e) == "[object Boolean]");
}
function tt(e) {
...
const gt = "Incorrect 'index' type",
  pt = (e) => `Invalid value for key ${e}`,
  Mt = (e) => `Pattern length exceeds max of ${e}.`,
  mt = (e) => `Missing ${e} property in key`,
...
function st(e) {
  let t = null,
    ...
    r = 1,
    i = null;
  if (E(e) || I(e)) (n = e), (t = Q(e)), (s = K(e));
  else {
    if (!U.call(e, "name")) throw new Error(mt("name"));
    const c = e.name;
    if (((n = c), U.call(e, "weight") && ((r = e.weight), r <= 0)))
      throw new Error(xt(c));
    (t = Q(c)), (s = K(c)), (i = e.getFn);
  }
  ...
  const r = (i, c, o) => {
    if (M(i))
      if (!c[o]) s.push(i);
        ...
        const a = i[h];
        if (!M(a)) return;
        if (o === c.length - 1 && (E(a) || q(a) || ft(a))) s.push(ut(a));
        else if (I(a)) {
          n = !0;
          for (let l = 0, f = a.length; l < f; l += 1) r(a[l], c, o + 1);
        } else c.length && r(a, c, o + 1);
      }
  };
...
const yt = { includeMatches: !1, findAllMatches: !1, minMatchCharLength: 1 },
  ...
  St = { location: 0, threshold: 0.6, distance: 100 },
  At = {
    useExtendedSearch: !1,
    getFn: Et,
    ignoreLocation: !1,
...
function Lt(e = 1, t = 3) {
  ...
  return {
    get(r) {
      const i = r.match(wt).length;
      if (s.has(i)) return s.get(i);
      const c = 1 / Math.pow(i, 0.5 * e),
  ...
  _addObject(t, s) {
    ...
    this.keys.forEach((r, i) => {
      ...
      if (M(c)) {
        if (I(c)) {
          let o = [];
          const h = [{ nestedArrIndex: -1, value: c }];
          for (; h.length; ) {
            const { nestedArrIndex: a, value: l } = h.pop();
            if (M(l))
              if (E(l) && !j(l)) {
                let f = { v: l, i: a, n: this.norm.get(l) };
                o.push(f);
              } else
                I(l) &&
                  l.forEach((f, d) => {
...
function nt(
  e,
  t,
  { getFn: s = u.getFn, fieldNormWeight: n = u.fieldNormWeight } = {}
) {
  const r = new V({ getFn: s, fieldNormWeight: n });
  return r.setKeys(e.map(st)), r.setSources(t), r.create(), r;
}
function Rt(
  e,
  { getFn: t = u.getFn, fieldNormWeight: s = u.fieldNormWeight } = {}
) {
  const { keys: n, records: r } = e,
    i = new V({ getFn: t, fieldNormWeight: s });
...
function bt(e = [], t = u.minMatchCharLength) {
  let s = [],
    n = -1,
    r = -1,
    i = 0;
...
function Nt(
  ...
  if (t.length > b) throw new Error(Mt(b));
  const l = t.length,
    f = e.length,
  ...
  for (let x = 0; x < l; x += 1) {
    ...
    for (; S < A; )
      v(t, {
        ...
        ? (S = A)
        : (k = A),
        (A = Math.floor((k - S) / 2 + S));
    k = A;
    let Y = Math.max(1, d - A + 1),
      P = c ? f : Math.min(d + A, f) + l,
      O = Array(P + 2);
    O[P + 1] = (1 << x) - 1;
    for (let _ = P; _ >= Y; _ -= 1) {
      let C = _ - 1,
  ...
  const F = { isMatch: p >= 0, score: Math.max(0.001, R) };
  if (m) {
    const x = bt(L, o);
    x.length ? h && (F.indices = x) : (F.isMatch = !1);
  }
  return F;
}
function Ot(e) {
...
class rt {
  constructor(
    ...
    if (
      ...
      return;
    const l = (d, g) => {
        this.chunks.push({ pattern: d, alphabet: Ot(d), startIndex: g });
      },
    ...
    if (f > b) {
      ...
      const g = f % b,
        p = f - g;
      for (; d < p; ) l(this.pattern.substr(d, b), d), (d += b);
      if (g) {
        const m = f - b;
  ...
  searchIn(t) {
    ...
    let l = [],
      f = 0,
      d = !1;
    this.chunks.forEach(({ pattern: p, alphabet: m, startIndex: L }) => {
      const {
        isMatch: y,
        score: N,
        indices: R,
    ...
    let g = { isMatch: d, score: d ? f / this.chunks.length : 1 };
...
const D = [$t, ct, Ct, vt, Ft, Tt, kt, it],
  J = D.length,
  Pt = / +(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)/,
  jt = "|";
function Kt(e, t = {}) {
...
const Dt = new Set([it.type, ct.type]);
class Wt {
  constructor(
    t,
    {
  ...
  searchIn(t) {
    ...
    for (let h = 0, a = s.length; h < a; h += 1) {
      ...
      for (let f = 0, d = l.length; f < d; f += 1) {
        const g = l[f],
          { isMatch: p, indices: m, score: L } = g.search(t);
        if (p) {
          if (((i += 1), (o += L), n)) {
            const y = g.constructor.type;
            Dt.has(y) ? (c = [...c, ...m]) : c.push(m);
          }
        } else {
          (o = 0), (i = 0), (c.length = 0);
          break;
...
const T = { AND: "$and", OR: "$or" },
  z = { PATH: "$path", PATTERN: "$val" },
  H = (e) => !!(e[T.AND] || e[T.OR]),
  zt = (e) => !!e[z.PATH],
  Ht = (e) => !I(e) && tt(e) && !H(e),
  Z = (e) => ({ [T.AND]: Object.keys(e).map((t) => ({ [t]: e[t] })) });
function ot(e, t, { auto: s = !0 } = {}) {
  const n = (r) => {
    let i = Object.keys(r);
    const c = zt(r);
    if (!c && i.length > 1 && !H(r)) return n(Z(r));
    if (Ht(r)) {
      const h = c ? r[z.PATH] : i[0],
        a = c ? r[z.PATTERN] : r[h];
      if (!E(a)) throw new Error(pt(h));
      const l = { keyId: K(h), pattern: a };
      return s && (l.searcher = B(a, t)), l;
    }
  ...
  return H(e) || (e = Z(e)), n(e);
}
function Vt(e, { ignoreFieldNorm: t = u.ignoreFieldNorm }) {
  e.forEach((s) => {
    let n = 1;
...
class $ {
  constructor(t, s = {}, n) {
    (this.options = { ...u, ...s }),
      ...
      (this._keyStore = new _t(this.options.keys)),
      this.setCollection(t, n);
  }
  setCollection(t, s) {
    if (((this._docs = t), s && !(s instanceof V))) throw new Error(gt);
    this._myIndex =
      s ||
  ...
  search(t, { limit: s = -1 } = {}) {
    ...
    return (
      Vt(h, { ignoreFieldNorm: o }),
      i && h.sort(c),
      q(s) && s > -1 && (h = h.slice(0, s)),
      Ut(h, this._docs, { includeMatches: n, includeScore: r })
    );
  }
  _searchStringList(t) {
    const s = B(t, this.options),
      { records: n } = this._myIndex,
      r = [];
    return (
      n.forEach(({ v: i, i: c, n: o }) => {
        if (!M(i)) return;
        const { isMatch: h, score: a, indices: l } = s.searchIn(i);
        h &&
          r.push({
            item: i,
            idx: c,
    ...
    const s = ot(t, this.options),
      n = (o, h, a) => {
        if (!o.children) {
          ...
          return g && g.length ? [{ idx: a, item: h, matches: g }] : [];
        }
        const l = [];
        for (let f = 0, d = o.children.length; f < d; f += 1) {
          ...
          else if (o.operator === T.AND) return [];
        }
        return l;
      },
    ...
    return (
      r.forEach(({ $: o, i: h }) => {
        if (M(o)) {
          let a = n(s, o, h);
          a.length &&
            (i[h] || ((i[h] = { idx: h, item: o, matches: [] }), c.push(i[h])),
            a.forEach(({ matches: l }) => {
              i[h].matches.push(...l);
            }));
        }
  ...
  _findMatches({ key: t, value: s, searcher: n }) {
    ...
    if (I(s))
      s.forEach(({ v: i, i: c, n: o }) => {
        if (!M(i)) return;
        const { isMatch: h, score: a, indices: l } = n.searchIn(i);
        h &&
          r.push({ score: a, key: t, value: i, idx: c, norm: o, indices: l });
      });
    else {
      const { v: i, n: c } = s,
        { isMatch: o, score: h, indices: a } = n.searchIn(i);
      o && r.push({ score: h, key: t, value: i, norm: c, indices: a });
    }
    return r;
  }
}
...
$.parseQuery = ot;
Bt(Wt);
export { $ as default, $ as fuse_esm_default };
